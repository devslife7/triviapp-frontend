...
const GameContainer = props => {
  ...
  useEffect(() => {
    fetch(`${serverURL}games/${props.location.state.gameId}`, {
      method: "GET",
      headers: {
      ...
      .then(data => {
        let questions = data.game.questions.map(newQuestion => {
          ...
          return {
            ...
            correct: fixedCorrect,
            incorrect1: fixedIncorrect1,
            incorrect2: fixedIncorrect2,
            incorrect3: fixedIncorrect3,
  ...
  const nextQuestion = (number, rightWrong, rightAnswer) => {
    ...
    if (index < questions.length - 1) {
      ...
      setTimeout(() => {
        ...
        fetch(`${serverURL}user_games/${localStorage.usergame}`, userGameConfig)
          .then(resp => resp.json())
  ...
  return (
    <div style={{ textAlign: "center" }}>
      {questions.length !== 0 ? (
        <QuestionContainer question={sendQuestion()} nextQuestion={nextQuestion} />
      ) : null}
